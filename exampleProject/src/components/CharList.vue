  <!--Template-->
  <template>
    <div v-if="loading" data-test="loading-div">Loading Gryffindor Characters...</div>

    <div v-else-if="error" data-test="error-div">{{ error }}</div>

    <ul v-else>
      <li v-for="char in charsWithImages" :key="char.id">
          <img class="profileImg" :src="char.image" :alt="`${char.name} image`"/>
        <h3>{{ char.name }}</h3>
      </li>
    </ul>
  </template>
